...
public var runningvm: Boolean = false
...
class Act {
    ...
    val data: Any
    var bind: Any
    constructor (n: Long, dt: Any, bd: Any) { num = n; data = dt; bind = bd }
}
...
// ----- AST-Interpreter
    ...
    fun eval() {
        ...
        ecall = 0
        do {  equit = true
            if (!runningvm) throw Exception("CALC-STOP")
            when (efun) {
...
// ----- primitive functions
    ...
    fun faa() {
        when (etop) {
            is Combine -> {  if ((etop as Combine).term is Cell) {
                             ...
                             } else etop = Error(idaa,"Cell als Term erwartet")
                          }
            is Error   -> { }
    ...
    */
    ...
    fun fdump() {
        ...
        var s: String = ""
        var ln: String = ""
        val crlf: String = ""+13.toChar()+10.toChar()
        while (p is Cell) {
            id = p.head as Ident
...
// ------- prelude text
    ...
    val crlf: String = ""+13.toChar()+10.toChar()
    val res: String =
    toValue(calc("trans==(isatom°head)->();(head aa),trans°(tail aa)"))+crlf+
    toValue(calc("distl==(trans°([0] make length°[1]) ee [1])°ee"))+crlf+
    toValue(calc("distr==(trans°[0] ee [1] make length°[0])°ee"))+crlf+
    toValue(calc("->* ==((head°term) app arg)->(term app (tail°term) app arg);arg"))+crlf+
    toValue(calc("map0==((isatom°[0]°[0])->();([1] app ((head°[0]),tail)°[0]),(((tail°[0]),tail)°[0]) map0 [1])°ee"))+crlf+
    toValue(calc("aa0==arg map0 head°term"))+crlf+
    toValue(calc("<- == (head°term) app reverse°[0]°(((isprop°[1]) and isprop°[2])->*(prop°(head°[2]),(head°[1]),[0],),(tail°[1]),(tail°[2]),)°(),(tail°term) ee arg"))+crlf+
    toValue(calc("isodd == (round imod '[2])='[1]"))+crlf+
    toValue(calc("ismat==isnum°head°head"))+crlf+
    toValue(calc("zero==ismat -> (length zeromat length°head) ; isreal -> 0 ; isint -> '[0] ; isbool -> 'false ; fail°'zero,id,"))+crlf+
    toValue(calc("one ==ismat -> (idmat°length) ; isreal -> 1 ; isint -> '[1] ; isbool -> 'true  ; fail°'one,id,"))+crlf+
    toValue(calc("scale==((isnum°[1]) -> ((* aa0) aa0) ; fail°'scale,id)°ee"))+crlf+
    toValue(calc("zeromat==(((([2] aa0)°(iota°[1]),tail) aa0)°(iota°round°[0]) ,(round°[1]),(zero°[0]),)°ee"))+crlf+
    toValue(calc("idmat  ==((((([0]=[1])->[4];[3]) aa0)°(iota°[1]),id) aa0)°(iota°round),round,zero,one,"))+crlf+
    toValue(calc("negifodd == ((isodd°[0])->(neg°[1]);[1])°ee"))+crlf+
    toValue(calc("droppos==((isnull°[0])->();(iszero°[1])->((tail°[0]) droppos pred°[1]);(head°[0]),(tail°[0]) droppos pred°[1])°ee"))+crlf+
    toValue(calc("droprow==(droppos aa)°distr"))+crlf+
    toValue(calc("det==((isnull°[1])->(head°[0]);[3]°(not°isnull°[0])->*(tail°[0]),[1],(succ°[2]),([3] + [4] negifodd (head°[0])*det°[1] droprow [2]),(succ°[4]),)°head,tail,'[0],(zero°head°head),'[0],"))+crlf+
    toValue(calc("Aij==(([0] droppos [1]°[1]) droprow [0]°[1])°ee"))+crlf+
    toValue(calc("inv==((iszero°[1])->('inv error \"Division by Zero\"); (((([1]°[1]) * (+°[0]) negifodd det°([0]°[1])Aij[0]) aa)aa)°(distr aa)°((distl aa)°[2]distr[2])distr[0]ee(one°[1])/[1])°id,det,('[0],iota°pred°round°length),"))+crlf+
    toValue(calc("add==(((ismat°[0]) and ismat°[1])->((((+ aa)°trans) aa)°trans);((isnum°[0]) and isnum°[1])->([0]+[1]);fail°'add,id)°ee"))+crlf+
    toValue(calc("sub==(((ismat°[0]) and ismat°[1])->((((- aa)°trans) aa)°trans);((isnum°[0]) and isnum°[1])->([0]-[1]);fail°'sub,id)°ee"))+crlf+
    toValue(calc("mul==(((ismat°[0]) and ismat°[1])->([0] MM [1]);((isnum°[0]) and ismat°[1])->([1] scale [0]);((ismat°[0]) and isnum°[1])->([0] scale [1]);((isnum°[0]) and isnum°[1])->([0]*[1]);fail°'mul,id)°ee"))+crlf+
    toValue(calc("fail=='FAIL error id"))+crlf+
    toValue(calc(":= == arg iput (head°term) ee (tail°term) app arg"))+crlf+
    toValue(calc("IP == (+ \\)°(* aa)°trans°ee"))+crlf+
    toValue(calc("MM==((IP aa)aa)°(distl aa)°distr°([0] ee trans°[1])°ee"))+crlf+
    toValue(calc("rnd==(((id roundto 5) aa) aa)"))+crlf+
    toValue(calc("pim == reverse°[2]°(([0]>1)->*(iszero°[0]-[1]*floor°[0]/[1])->(([0]/[1]),[1],([1],[2]),);([0],([1]+1),[2],))°id,2,(),"))
    return res  }
...
fun main() {      //   help:()     new ?      help:name    save:datei1.txt     load:datei1.txt     ???
    ...
    println(vm.toValue(vm.calc("help == isnull-> 'nix ; info")))
    println(vm.toValue(vm.calc("(true->*(id>0)->pred;stopvm)°1000")))
